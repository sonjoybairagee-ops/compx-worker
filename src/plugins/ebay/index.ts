 ...
 * `input.maxResults` was never declared or read anywhere in this file.
 * SerpApi's eBay engine returns however many organic_results it has for
 * the keyword, and every one got saved AND charged for regardless of what
 * the user actually requested via the Discover page's "Max Results"
 * control. Fixed the same way: clamp + slice after both the fresh-fetch
 * and cache-hit paths converge, and — specifically for this plugin —
 * AFTER the seller feedback/sold-count filters run, not before. Slicing
 * first would risk handing back fewer than maxResults items even when
 * more filter-matching results existed further down the list; the cap
 * should apply to the filtered set the user actually wants, not the raw
 * unfiltered one.
 */
...
import { ebayRouter } from "./router.js";
...
// FIX: same conservative default as plugins/amazon — keeps a request that
// omits maxResults entirely from turning into "no limit".
...
async function run(ctx: PluginContext): Promise<PluginResult> {
  ...
  // FIX: same clamp as plugins/amazon — explicit finite/positive check
  // rather than `||`, so an intentional (if unusual) value isn't lost to
  // falsy-coercion, and a missing/invalid value falls back to the default
  // instead of meaning "unlimited".
  const maxResults =
  ...
  if (cacheHit.hit && cacheHit.data) {
    ...
    try {
      ...
      const { data: apifyData, provider } = await ebayRouter.fetch(
        { keyword: input.keyword, filters: input.ebayFilters },
        { jobId: ctx.jobId, logger, redis: ctx.redis }
      );
      await logger.log(`eBay data served by provider "${provider}".`);
  ...
  // FIX: enforce maxResults here — after filtering, so the cap applies to
  // the set the user's criteria actually matched, not the raw unfiltered
  // fetch. This is also the point where the fresh-fetch and cache-hit
  // paths have already converged, so neither path can bypass it.
