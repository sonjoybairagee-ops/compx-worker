...
import { linkedinRouter } from "./router.js";
...
async function run(ctx: PluginContext): Promise<PluginResult> {
  ...
  if (cacheHit.hit && cacheHit.data) {
    ...
    try {
      await logger.log(`Fetching ${urls.length} profile(s) from LinkedIn provider...`);
      const { data: apifyData, provider } = await linkedinRouter.fetch(
        { profileUrls: urls },
        { jobId: ctx.jobId, logger, redis: ctx.redis }
      );
      await logger.log(`LinkedIn data served by provider "${provider}".`);
