...
import { fetchTripadvisorSerpApi } from "./serpapi.js";
...
async function run(ctx: PluginContext): Promise<PluginResult> {
  ...
  if (cacheHit.hit && cacheHit.data) {
    ...
    try {
      ...
      const apifyData = await fetchTripadvisorSerpApi(input.keyword, input.location, logger, ctx.redis, input.tripadvisorFilters);
